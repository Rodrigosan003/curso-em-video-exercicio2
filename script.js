function verificar() {
   // alert('funcionou!')
    var data = new Date()
    var ano = data.getFullYear()
    var fano = document.getElementById('txtano')
    var res = document.getElementById('res')
    
    if(fano.value.length == 0 || Number(fano.value) > ano){
        alert('[ERRO] Verifique os dados e tente novamente!')
    } else{
        //alert('tudo ok')
        var fsex = document.getElementsByName('radsex')
        var idade = ano - Number(fano.value)
        var gênero = ''
        var img = document.createElement('img')
        img.setAttribute('id', 'foto')
        if (fsex[0].checked) {
            gênero = 'Homem'
            if(idade >= 0 && idade < 10){
                //Criança
                img.setAttribute('src', '/img/fotomenino.png')
            } else if(idade < 21){
                //Jovem
                img.setAttribute('src', '/img/fotohomemjovem.png')
            } else if(idade < 50){
                //Adulto
                img.setAttribute('src', '/img/fotohomemadulto.png')
            } else{
                //Idoso
                img.setAttribute('src', '/img/fotoidoso.png')
            }
        } else if (fsex[1].checked) {
            gênero = 'Mulher'
            if(idade >= 0 && idade < 10){
                //Criança
                img.setAttribute('src', '/img/fotomenina.png')
            } else if(idade < 21){
                //Jovem
                img.setAttribute('src', '/img/fotomulherjovem.png')
            } else if(idade < 50){
                //Adulto
                img.setAttribute('src', '/img/fotomulheradulta.png')
            } else{
                //Idoso
                img.setAttribute('src', '/img/fotoidosa.png')
            }
        }
        res.style.textAlign = 'center'
        res.innerHTML = `Detectamos ${gênero} com ${idade} anos.`
        res.appendChild(img)
    }
}